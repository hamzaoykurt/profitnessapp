-- Count set-based completions as streak activity for leaderboard RPCs.
-- Some workout flows sync set_completions before workout_logs, so using both
-- sources keeps the streak ranking aligned with actual completed work.

create index if not exists set_completions_user_date_idx
  on public.set_completions (user_id, date desc);
...
as $function$
  with dates as (
    ...
    union
    select sc.user_id, sc.date::date as day
