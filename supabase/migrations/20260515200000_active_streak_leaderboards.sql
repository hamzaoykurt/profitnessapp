-- Rank the active daily streak shown in the app, not the all-time best streak.

create index if not exists user_stats_current_streak_idx
  on public.user_stats (current_streak desc, user_id);

drop function if exists public.get_leaderboard_streak(integer);
drop function if exists private.get_leaderboard_streak(integer);
drop function if exists public.get_my_rank_streak();
drop function if exists private.get_my_rank_streak();
drop function if exists public.get_friend_leaderboard_streak(integer);
drop function if exists private.get_friend_leaderboard_streak(integer);
...
as $function$
  with dates as (
    select wl.user_id, wl.date::date as day
    from public.workout_logs wl
    group by wl.user_id, wl.date::date
  ),
  ordered as (
    select
      d.user_id,
