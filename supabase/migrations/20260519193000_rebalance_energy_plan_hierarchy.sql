-- Rebalance the Energy economy around clearer plan hierarchy.
-- Free includes core tracking only; AI usage spends visible Energy.
-- Oracle chat now behaves as a bundle: 1 Energy opens 10 chat messages.

update public.billing_products
   set title = replace(replace(title, 'AI Kredisi', 'Enerji'), 'AI Credits', 'Energy')
...
values
    ('credits_10', 'credit_pack', null, 10, '10 Enerji', '₺29', 30, '{}'::jsonb),
    ('credits_25', 'credit_pack', null, 25, '25 Enerji', '₺59', 31, '{}'::jsonb),
    ('credits_50', 'credit_pack', null, 50, '50 Enerji', '₺99', 32, '{"badge":"EN POPÜLER"}'::jsonb),
    ('credits_100', 'credit_pack', null, 100, '100 Enerji', '₺169', 33, '{}'::jsonb),
    ('credits_200', 'credit_pack', null, 200, '200 Enerji', '₺299', 34, '{}'::jsonb),
    ('credits_500', 'credit_pack', null, 500, '500 Enerji', '₺649', 35, '{}'::jsonb)
...
begin
    ...
    case p_tool
        when 'ORACLE_CHAT' then
            v_period := 'day';
            v_credit_cost := 1;
            v_free_limit := case v_plan when 'ELITE' then 10000 when 'PRO' then 250 else 0 end;
        when 'PROGRAM_GENERATE_TEXT' then
            v_period := case when v_plan = 'FREE' then 'lifetime' else 'month' end;
            v_credit_cost := 6;
            v_free_limit := case v_plan when 'ELITE' then 160 when 'PRO' then 30 else 0 end;
        when 'PROGRAM_GENERATE_MEDIA' then
            v_period := case when v_plan = 'FREE' then 'lifetime' else 'month' end;
            v_credit_cost := 10;
            v_free_limit := case v_plan when 'ELITE' then 90 when 'PRO' then 12 else 0 end;
        when 'PROGRAM_EDIT' then
            v_period := 'month';
            v_credit_cost := 4;
            v_free_limit := case v_plan when 'ELITE' then 160 when 'PRO' then 40 else 0 end;
        when 'WEIGHT_TREND_ANALYSIS', 'EXERCISE_PROGRESS_ANALYSIS', 'WORKOUT_PROGRESS_ANALYSIS' then
            v_period := 'day';
            v_credit_cost := 2;
            v_free_limit := case v_plan when 'ELITE' then 10000 when 'PRO' then 20 else 0 end;
    ...
    if p_tool = 'ORACLE_CHAT' and ((v_used - v_free_limit) % 10) <> 0 then
        v_credit_cost := 0;
    end if;
    ...
    if v_credit_cost > 0 and v_balance < v_credit_cost then
        insert into public.ai_usage_events (
        ...
        return jsonb_build_object(
            ...
            'message', 'Bu işlem için Enerji veya abonelik gerekiyor.',
    ...
    values (p_user_id, p_tool, 'reserved', 'credit_balance', v_credit_cost, v_period, v_period_start, p_idempotency_key)
    returning * into v_event;
