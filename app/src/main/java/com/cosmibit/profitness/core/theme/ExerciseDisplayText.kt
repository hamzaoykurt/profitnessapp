...
private val ExerciseNameTerms = listOf(
    ...
    LocalizedFitnessTerm("Seated Cable Row", "Oturarak kablo row"),
    LocalizedFitnessTerm("Wide Grip Seated Row", "Geniş tutuş oturarak row"),
    LocalizedFitnessTerm("Wide Grip Cable Row", "Geniş tutuş kablo row"),
    ...
    LocalizedFitnessTerm("Cable Curl", "Kablo curl"),
...
private val FitnessGroupTerms = listOf(
    ...
    LocalizedFitnessTerm("Mobility", "Mobilite", setOf("mobilite")),
...
/** A single-language label for dense surfaces such as workout cards. */
fun AppThemeState.exerciseCompactDisplayName(name: String, nameEn: String = ""): String {
    val knownFromName = ExerciseNameTerms.findTerm(name)
    val knownFromEnglish = nameEn.takeIf { it.isNotBlank() }?.let { ExerciseNameTerms.findTerm(it) }
    val term = knownFromEnglish ?: knownFromName
    val english = term?.english ?: nameEn.takeIf { it.isNotBlank() } ?: name
    return if (language == AppLanguage.ENGLISH) {
        ...
        term?.turkish?.takeIf { it.isNotBlank() } ?: name.ifBlank { english }
    }
}
...
/** A single-language fitness term for compact, glanceable UI. */
fun AppThemeState.fitnessTermCompactDisplayName(value: String): String {
    val term = FitnessGroupTerms.findTerm(value) ?: return value
    return if (language == AppLanguage.ENGLISH) term.english else term.turkish
}
